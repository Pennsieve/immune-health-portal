...
export function createIntakeToken(
  ...
  email: string,
  secret: string,
  ttlSeconds = 2592000, // 30 days — the full intake takes real effort to complete
): string {
...
export function verifyIntakeToken(token: string, secret: string): IntakeTokenPayload {
  const parts = token.split('.')
  if (parts.length !== 3) throw new Error('malformed token')
  const [header, body, sig] = parts
  const expected = createHmac('sha256', secret).update(`${header}.${body}`).digest('base64url')
  if (sig !== expected) throw new Error('invalid signature')
  const payload = JSON.parse(decode(body)) as IntakeTokenPayload
  if (payload.exp < Math.floor(Date.now() / 1000)) throw new Error('token expired')
  return payload
}
